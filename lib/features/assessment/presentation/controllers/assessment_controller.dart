...
class AssessmentController extends ChangeNotifier {
  ...
  double get progressPercentage => _currentQuestionIndex / _questions.length;
  ...
  AssessmentResult calculateResult() {
    return AssessmentCalculator.calculateResult(
  ...
  /// 모든 질문이 답변되었는지 확인
  bool get isAllQuestionsAnswered => _answers.length == _questions.length;

  /// 답변률 계산
  double get answerPercentage => _answers.length / _questions.length;
}
